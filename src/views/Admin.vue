...
<template>
  <el-form :model="form" label-width="auto" style="max-width: 800px">
    ...
    >
      <template #item="{ element: unit, index: unitIndex }">
        <div :key="'unit-' + unitIndex" class="visitor-unit">
          <el-card shadow="hover">
            <div class="unit-header">
              <div class="drag-handle-container">
                <el-icon class="unit-drag-handle"><Rank /></el-icon>
            ...
            <!-- 人员列表 - 使用draggable组件包裹 -->
            <draggable
              v-model="unit.members"
              item-key="id"
              handle=".member-drag-handle"
              group="members"
              @end="onMemberDragEnd(unitIndex)"
              class="members-container"
            >
              <template #item="{ element: member, index: memberIndex }">
                <div :key="'member-' + memberIndex" class="member-item">
                  <el-card shadow="never" class="member-card">
                    <div class="member-header">
                      <div class="drag-handle-container">
                        <el-icon class="member-drag-handle"><Rank /></el-icon>
...
const addUnit = () => {
  form.units.push({
    name: '',
    members: [{ name: '', position: '', plateNumber: '' }],
  })
}
...
const onMemberDragEnd = (unitIndex: number) => {
  // 人员顺序已经自动更新，无需额外操作
}
